...
export function TabsWithSlider({
  ...
  return (
    <div className={classNames('relative flex gap-2', className)}>
      ...
      {tabs.map((tab, index) => (
        <button
          ...
          className={classNames(
            ...
            tab.id === activeTab
              ? classNames('text-bolt-elements-textPrimary dark:text-white', activeTabClassName)
              : classNames(
                  'bg-transparent text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary hover:bg-bolt-elements-background-depth-3/50',
                  tabClassName,
                ),
          )}
        >
      ...
      {/* Animated slider */}
      <motion.div
        className={classNames('absolute bottom-0 left-0 h-10 rounded-lg bg-transparent border border-bolt-elements-borderColor -z-10', sliderClassName)}
        initial={false}
        animate={{
