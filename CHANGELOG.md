...
- Preview asset requests now wait up to 30.2 seconds through bounded same-session port handoffs and transient Vite compile responses during file synchronization. If Vite exits during runtime preparation, the project retains its private port assignment while automatic recovery replaces the process; another session's port still fails closed. A healthy same-origin iframe whose app root remained blank also receives one revision-scoped reload instead of staying empty after a transient handoff.
