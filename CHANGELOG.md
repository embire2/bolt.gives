...
- Accept Microsoft's current ID Verified Artifact Signing AOC/EOC 03/04 issuer rotations alongside the pinned legacy hierarchy. Every path still requires its complete matching PCA/root chain, the bolt.gives publisher identity, code-signing usage and an RFC3161 timestamp.
...
- Prevented persistent healthy Preview dev servers from starving managed Cloudflare fleet refreshes. Automatic rollouts still yield to foreground commands, Preview recovery, and runtime-node provisioning, but idle open Previews no longer leave new provisioning blocked behind a stale live release. Registry-changing rollout, registration, refresh, expiry, and suspension operations are now serialized against freshly loaded state so a background fleet write cannot erase a newly provisioned client assignment. Startup also restores assignments present only in the private admin database without overwriting current registry records or client session hashes.
...
- Fixed model-authored background Preview checks, including commands prefixed with `cd /home/project`, appending Vite host/port flags to `curl` or `head`, falsely failing a healthy generated app, and opening a hidden Safe Auto approval. Recognized dev-server commands now run as managed Preview starts while the runtime owns health verification.
...
- Replaced unavailable settings and repository icon names with bundled equivalents, removing build-time asset warnings and preventing blank controls.
...
- `pnpm run lint`
- `pnpm run typecheck`
- `pnpm test` (`179` files passed, `1,029` tests passed, `3` skipped)
- `pnpm check:boundaries:strict`
- `pnpm run build`
...
- Saved projects now reopen with their complete visible conversation, source snapshot, and original hosted runtime session. History hydration uses the authoritative `/chat/:id` route parameter even when shared loader context is empty; snapshot reads use the chat record's internal ID, first saves allocate that ID before writing files, writes finish before navigation, and delayed samplers cannot leak messages into a different project. A healthy saved hosted Preview is rebound through a read-only status check instead of rerunning setup/start commands. Historical actions remain visible as completed transcript entries but no longer rerun file writes, installs, or preview starts, and restored execution events no longer force the visible surface away from Chat.
...
- Streamed hosted file actions now use debounced non-pruning batches between command boundaries, followed by one authoritative full snapshot before install/start. Partial browser snapshots can no longer delete earlier starter files or race Vite into a false `missing index.html` failure.
- Hosted start actions no longer pre-kill the active Preview before invoking the runtime. The runtime can now reuse an unchanged healthy Vite process as designed, avoiding needless repair flashes, restarts, and Preview revision churn during follow-up continuations.
...
- Preview startup now waits through a bounded strict-health window instead of killing Vite on its first transient compile response. Asset requests also wait up to 30.2 seconds through same-session port handoffs and synchronized-file settling. If Vite exits during runtime preparation, the project retains its private port assignment while automatic recovery replaces the process; another session's port still fails closed. A healthy same-origin iframe whose app root remained blank also receives one revision-scoped reload instead of staying empty after a transient handoff.
...
- Streamed Magnet Claude message-start events receive the missing `output_tokens: 0` compatibility field without buffering, preventing valid Opus, Sonnet, and Fable output from failing strict SDK validation before file actions run.
- Hosted Claude build requests copy the Bolt contract, current workspace snapshot, project memory, and latest objective into the latest user turn because Magnet's Claude-compatible backend does not reliably honor the Anthropic `system` field; earlier conversation history remains intact.
- Magnet Claude's native `write_file` stream is bridged server-side into the existing Bolt file-action protocol, so Opus, Sonnet, and Fable can apply project edits without direct filesystem credentials or browser-visible operator secrets.
- Existing-file edits from Magnet Claude models now require a compact exact `find`/`replace` tool payload; the server reconstructs the complete trusted workspace file before applying it, avoiding completion-limit truncation and redundant recovery passes on normal follow-up prompts. Empty workspaces retain complete-file creation for bootstrap.
...
- Hosted FREE build instructions now constrain existing-project work to one focused file per step and avoid rewriting an already-satisfied starter, reducing oversized provider streams without losing incremental follow-up capability.
- Hosted FREE instructions now define Bolt XML as the file-operation transport and explicitly prevent Claude coding profiles from refusing valid edits because native Read/Edit/Bash tools are absent; the included project snapshot is treated as already-read context.
...
- Hosted FREE requests now use the bounded 120-second client recovery policy instead of inheriting the 260-second BYOK long-think allowance solely from the model name.
...
- Hosted preview status reconciliation now preserves correctly assigned cross-origin and in-flight iframe loads. It no longer aborts and reloads the same healthy preview on every status event, eliminating visible flashing and repeated runtime proxy traffic.
- README screenshot release checks now validate the current spam-safe `Contribute on GitHub` pathway through shared, regression-tested homepage markers and report missing markers directly instead of timing out on retired application-form wording.
