...
- Existing-file edits from Magnet Claude models now use a compact exact `find`/`replace` tool payload; the server reconstructs the complete trusted workspace file before applying it, avoiding completion-limit truncation and redundant recovery passes on normal follow-up prompts.
