...
- Preview asset requests now wait up to 30.2 seconds through bounded same-session port handoffs, including the brief restart interval where the port reservation is intact but preview metadata is being rebuilt. Another session's port still fails closed, and a healthy same-origin iframe whose app root remained blank receives one revision-scoped reload instead of staying empty after a transient handoff.
