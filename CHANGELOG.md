...
- The browser now reads the server-advertised FREE stream deadline and enforces it as an absolute recovery boundary; incoming data counters are read through stable refs so hidden reasoning, commentary, or telemetry events cannot restart the deadline sampler.
