...
- [~] B13: A signed per-project HTTPS Preview gateway is implemented. The 13 September combined rootless browser run passed interactive Preview, blocked platform storage/document/cookie access, server-side 403 for a cross-origin mutation, follow-up and history/restart continuity. Bounded project-only browser-error reporting is implemented too. Public TLS/routing, WebSockets, database/mobile acceptance and staging migration remain open; local self-signed HTTPS does not certify production. See [runtime isolation rollout](docs/operations/runtime-isolation.md).
- [~] B14: Rootless per-project Podman execution is implemented, including single-project mounts, resource limits, loopback-only Preview ports, awaited container teardown and rejection of operator-directory session IDs. Real container tests deny root/operator/sibling-file access and host-loopback connections; combined FREE browser acceptance passes. The live runtime still uses its old identity/storage. Preserve source/database records, verify privileged control-plane operations and migrate staging before production; do not treat the local fixture as a live migration.
...
| [~] B03a / B03b | Fixed SSR and double-submit/retry; browser login/error/cancel fixture passes; real Stripe test-mode fulfillment remains   | surfaces/control-plane       | Direct and client navigation return 200; Checkout does not loop; cancellation/retry work                                  |
...
| [~] B09a / B09b | Implemented server-confirmed billing display and bounded pending checks; expired/forged success denied      | control-plane/surfaces | Real Stripe test-mode fulfillment remains open                                             |
...
### Shipped in v3.0.9.29

- [x] Normalize hosted preview checkpoints, command-ready previews, preview-status payloads, preview-events, and Workbench preview sync to the current browser origin.
- [x] Prevent healthy Pages-generated apps from falling back to blocked `https://bolt.gives/runtime/preview/...` iframe URLs.
- [x] Add regression coverage for the Pages central-preview URL normalization failure.

## v3.0.9.28 - Shipped Patch
...
- [x] The old `bolt.gives/contribute` application form was retired after spam pressure; contributors now use GitHub issues and pull requests.
