...
- GitHub release: `https://github.com/embire2/bolt.gives/releases/tag/v3.0.9.29`
- Linux installer: `https://raw.githubusercontent.com/embire2/bolt.gives/v3.0.9.29/install.sh`
- In progress: `v3.1.0`

`v3.0.9.29` is the current stable hosted and Linux self-host release. It keeps the focused Preview/Code workspace improvements, the deterministic Google Calendar first pass, the Cloudflare Pages collaboration transport fix, automatic runtime-node CLI provisioning, project publishing to bolt.gives subdomains, hosted FREE blocked-shell recovery, same-origin `bolt-gives.pages.dev` runtime proxying, shell-before-manifest Architect recovery, and empty-context first-pass recovery, then fixes Cloudflare Pages preview checkpoints so healthy generated apps do not fall back to blocked `https://bolt.gives/runtime/preview/...` iframe URLs.
