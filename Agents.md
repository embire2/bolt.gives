...
- GitHub release: `https://github.com/embire2/bolt.gives/releases/tag/v3.0.9.30`
- Linux installer: `https://raw.githubusercontent.com/embire2/bolt.gives/v3.0.9.30/install.sh`
- In progress: `v3.1.0`

`v3.0.9.30` is the current stable hosted and Linux self-host release. It keeps the focused Preview/Code workspace improvements, the deterministic Google Calendar first pass, the Cloudflare Pages collaboration transport fix, automatic runtime-node CLI provisioning, project publishing to bolt.gives subdomains, hosted FREE blocked-shell recovery, same-origin `bolt-gives.pages.dev` runtime proxying, shell-before-manifest Architect recovery, empty-context first-pass recovery, and Pages preview checkpoint fix, then stops normal hosted file-revision updates from remounting or cache-busting the Preview iframe while the agent is coding.
