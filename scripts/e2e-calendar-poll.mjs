...
await ta.fill(
  `Build a tiny React calendar app with a month grid and an Add Event button. The root <h1> must contain the exact string "${token}". Keep it to 5 small files max.`,
);
...
while (Date.now() < deadline) {
  ...
  if (tail !== lastBodyTail) {
    ...
    if (m) {
      console.log(`[+${t()}s] ui-status: ${m[0].slice(0, 240)}`);
    }
  }
...
await fs.writeFile(
  path.join(outDir, 'poll-final-body.txt'),
  (await page
    .locator('body')
    .innerText()
    .catch(() => '')) || '',
