...
The browser no longer creates speculative revisions immediately after a file sync. Initial loads, follow-up refreshes, and repair handoffs wait for the server's health-verified revision, and every handoff response carries the same cross-origin isolation headers as the generated app. Same-session Vite assets receive a bounded 30.2-second ownership-handoff window, with one revision-scoped blank-frame reload as a final browser fallback. This removes transient blocked-response failures while preserving the stable Preview frame.
