...
**Still open:** production isolation migration and rollback, scalable Preview TLS/DNS configuration, the remaining acceptance matrix, and the earlier intermittent B15 hook crash. Passing repeats alone do not prove that crash's root cause. Native Windows work remains a separate release lane. See [ROADMAP.md](ROADMAP.md) before promoting the validation branch.
