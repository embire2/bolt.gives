...
- New and restored projects enter Agent Mode automatically instead of asking users to choose between competing Chat and Workspace surfaces.
- Desktop keeps a focused conversation column beside a dominant Code/Preview workspace. Mobile uses a deliberate Agent/App switch and never unmounts the follow-up prompt.
- The compact composer exposes Agent, Plan first, and Run plan behavior plus frictionless switching between the four hosted FREE coding models.
...
![Pulseboard generated, improved, and published through the v4.0.0 Cloudflare and PostgreSQL E2E](docs/screenshots/pulseboard-cloudflare-e2e-v4.0.0.png)
...
- Transient MagnetAPI outages are rechecked after five seconds instead of blocking automatic repair for a full minute; actual rate-limit and exhausted-credit responses retain conservative backoff.
- Hosted FREE follow-ups no longer spend two model requests summarizing history and choosing files before coding. The agent compacts persisted project memory, keeps only the latest instruction, and selects relevant files from the authoritative runtime snapshot deterministically, reducing latency and preventing stale Preview errors from bloating later prompts.
- Model-authored background checks such as `cd /home/project && pnpm run dev & sleep ... curl ...` are reduced to one managed Preview start. Vite flags can no longer land on a trailing `head` process, Safe Auto does not hide a harmless dev-server approval, and server-side health probes remain the source of truth.
...
From the hosted Preview toolbar, users can publish the current project to a free address such as `https://acme-dashboard.instances.bolt.gives`. The runtime creates a production build, stages only deployable files, adds an advanced-mode Pages Worker for asset and deep-link handling, uploads it to the protected Cloudflare account, activates DNS/Caddy routing, and verifies both the Worker and public hostname before reporting success. The resulting site remains online independently of the editable Preview process.

Production staging recovers safe local entry assets that tools such as Vite can leave outside `dist`, but fails closed if an entry point references a missing file or a private file such as `.env.local`. Database credentials are never copied into the Cloudflare artifact. The project's dedicated PostgreSQL database remains durable and linked to its runtime identity for later coding sessions and server-side commands.
...
BOLT_CLOUDFLARE_PROJECT_DOMAIN_ROOT=instances.bolt.gives
...
BOLT_PROJECT_DATABASE_ADMIN_URL=<server-only-postgresql-provisioner-url>
...
The full prompt experience stays available in Agent Mode. Provider/model controls, attachments, web research, prompt enhancement, speech, mode toggles, save/resume/share, and the built-in web research note remain available without competing Chat/Workspace navigation. Google Calendar-style prompts start from a deterministic React/CSS Calendar Planner with a week grid, agenda panel, create-event action, and any explicit visible heading text requested by the user.
...
Every normal hosted project first receives a local, durable PostgreSQL database and restricted role on the application runtime server. The runtime provisions it before the first project command and injects `DATABASE_URL`/`PG*` only into that project's server-side command and Preview environment. Passwords are stored under the runtime workspace root with mode `0600`; browser state exposes only connection status and non-secret database identity.

`/workspace-setup` additionally creates a real project workspace on an operator-configured Ubuntu runtime node. The runtime server connects over SSH from the server side only, creates one Linux user per project, locks the workspace directory to that user, and returns one-time CLI credentials. If that optional node is unavailable, local project generation, Preview, and PostgreSQL continue instead of failing the coding run.
...
![bolt.gives Desktop v1.10.2 native Workspace with source files, live Preview, execution activity, and terminal](docs/screenshots/desktop-v1.10.2.png)

The installer creates Start Menu and Desktop shortcuts. First launch asks the user to pin the running app to the taskbar because Windows requires user approval for taskbar pins. The client checks the independent `desktop-v*` channel and streams update progress. Desktop v1.10.2 uses a separately signed elevated updater that verifies the exact release URL, declared size, SHA-256, Authenticode signature, and pinned signing hierarchy before requesting administrator approval. It closes the requesting app before replacement, verifies the installed result, and restores a verified backup if installation or post-install validation fails. A compatibility handoff also relaunches 1.10.2 after supported 1.0.x clients complete their mandatory update.
...
The Windows UI is native C#/.NET 8 WPF/XAML and does not load the hosted bolt.gives interface. WebView2 is used only for generated project Preview, with browser data kept in the current user's writable local application-data directory. Native surfaces cover six-digit email-code authentication, profile-isolated project history, full Chat, compact Workspace prompting, selectable FREE models, source editing, diff, terminal, Preview health and repair, FREE/Cloudflare/Custom Domain deployment, isolated Live CLI provisioning, Agent-token state, and settings. Project metadata, full chat history, runtime identity, model, source state, and project memory persist across restarts. Long-lived Chat, terminal, and Preview streams never perform synchronous network reads on the WPF dispatcher. Generated workspace actions execute natively in model order while the response streams: project-relative files sync atomically, shell mutations are guarded, runtime caches stay out of the source tree, and Preview start waits for the hosted stream before reusing a server-verified process or requiring a successful client start plus a ready event.

Private Windows CI passes 49 native tests and verifies signed application launch, installation, successful replacement, deliberate post-install failure, rollback, and legacy-updater relaunch. A real Windows Server RDP pass also created and improved a focus-timer project, interacted with its restored Preview, ran its project terminal, switched models without losing history, restarted the client with state intact, and published the result through both the FREE subdomain and protected Cloudflare paths.
...
Managed Cloudflare instances are registration-first, one-client / one-instance environments. Active and recoverable failed instances are refreshed from the current release SHA by the runtime rollout controller, automatic fleet refreshes are serialized to avoid overlapping startup/interval deployments, and every registry-changing registration/refresh/suspension operation reloads authoritative state before writing so concurrent fleet work cannot erase a new assignment. On startup, the file registry also recovers genuinely missing assignments from the private admin database while preserving current records and client session hashes. Refreshes are health-verified before being marked active, and failed rollouts retain last-good SHA/deployment metadata for rollback decisions. New instances are spawned from the same live build plus the protected hosted FREE relay secret, and release validation creates a fresh instance through `https://create.bolt.gives` before verifying preview plus follow-up prompt behavior.

The operator surface at `admin.bolt.gives` includes client profile filtering/export, managed instance assignment state, fleet summary cards, deployment history, last-good SHA, healthcheck and rollback outcome visibility, SMTP configuration, audience-based email sends, bug reports, and rollout guard visibility. Source-only rsync deployments can roll out only when their root-controlled `BOLT_RELEASE_SHA` exactly matches remote `main`; a stale or unverifiable marker fails closed. Self-hosting supports custom app/admin/create domains, local PostgreSQL, `psql`, operator credential seeding, Caddy-managed HTTPS, and a committed installer smoke command.

## Roadmap to v4.1.0

`v4.1.0` continues the previously planned v3.6 platform-hardening track after the v4.0 Agent Mode release. The focus is completing Custom Domain account and billing lifecycle, stronger operator-visible resource controls, installer resilience, and continued server-side runtime offload.
...
- New and restored projects enter one persistent Agent Mode where follow-up prompts stay visible beside files and Preview.
- Desktop Agent Mode gives the generated app most of the screen; mobile users switch explicitly between Agent and App while the prompt remains mounted.
...
- Hosted chat-created projects auto-provision a local isolated PostgreSQL database before their first command and can additionally provision a runtime-node CLI workspace in the background.
- Published projects receive `https://{subdomain}.instances.bolt.gives` backed by a persistent Cloudflare Pages Worker, or can be upgraded to a payment-verified Custom Domain project.
