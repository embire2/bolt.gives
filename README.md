...
- The migration is behavior-preserving: the full 1,026-test suite and the production bundle gate validate the unified build output.
...
- Deterministic first-party starters always continue through the selected hosted FREE model once Preview is healthy. The strict Calendar E2E now requires both the rendered app and a successful model request, so a starter-only preview cannot be reported as a completed agentic build.
