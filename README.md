...
**Live staging update, 13 September:** alpha1 now runs the isolated development build with non-root services, rootless project containers and publicly trusted per-project HTTPS Preview. A normal FREE/Luna Calendar browser journey passed first Preview, follow-up and saved-history restoration. The Upgrade button opened a real USD 5 monthly Stripe Checkout; it was expired without paying. Signed webhook rejection/replay and transactional billing tests pass. [Read the acceptance evidence and remaining work](docs/quality/2026-09-13-alpha-acceptance.md). **Production and the fleet remain on the stable release pending coordinated rollout.** Paid fulfillment, Windows/WSL reboot-resume and native desktop parity are not certified.
...
**Isolation and recovery development:** the earlier [1,347-test checkpoint](docs/quality/2026-09-13-isolation-checkpoint.md) includes real FREE/Luna interaction, cross-origin access rejection, follow-up/history/restart and injected-failure recovery. The latest full suite passed 1,360 tests with nine skipped before subsequent staging fixes. Public staging also exposed and fixed recursive runtime proxying, unauthenticated health verification and command-stream error handling. The intermittent B15 hook crash remains under investigation; passing repeats alone do not establish its root cause.
