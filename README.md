...
- Model-authored background checks such as `pnpm run dev & sleep ... curl ...` are reduced to one managed Preview start. Vite flags can no longer land on a trailing `head` process, Safe Auto does not hide a harmless dev-server approval, and server-side health probes remain the source of truth.
