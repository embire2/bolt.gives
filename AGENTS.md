...
The v4.1 reliability work is authorized through Phase 2. Alpha uses `/srv/bolt-gives-alpha-isolated` and `/srv/bolt-gives-alpha-isolated-workspaces`, non-root services and public Preview TLS. Originals remain rollback targets. Production traffic remains on `/srv/bolt-gives`; the separate `/srv/bolt-gives-isolated` application, workspace copy and `/etc/bolt-gives/production-isolated.env` are prepared, not promoted. Source/private-data checksums passed on 14 September; rebuildable dependency caches were pre-staged separately. Never rerun the fresh-copy migration over either target. Preserve `/srv/bolt-gives-production-checkpoints/20260914-pre-isolation`; reconcile subsequent production writes before promotion. Consult `docs/quality/2026-09-14-release-preparation.md`, `docs/operations/runtime-isolation.md` and `ROADMAP.md`. Staging success is not a fleet rollout. `main` pushes trigger Cloudflare deployment. Public `install.ps1` bootstraps the server through WSL, not private desktop source. Preserve passwords, configuration and customer data. Preview snapshots are read-only reconciliation; current runtime source is authoritative on reload, with cached fallback only after an explicit missing-session response.
