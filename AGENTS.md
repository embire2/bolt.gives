...
The v4.1 reliability work is authorized through Phase 2. Alpha now uses `/srv/bolt-gives-alpha-isolated` and `/srv/bolt-gives-alpha-isolated-workspaces`, non-root services and publicly trusted per-project Preview TLS. Original alpha trees are retained for rollback; do not rerun the copy migration over the active tree. Production and fleet migration remain pending. Consult `docs/quality/2026-09-13-alpha-acceptance.md`, `docs/operations/runtime-isolation.md` and `ROADMAP.md`; staging success is not a fleet rollout. `main` pushes trigger Cloudflare deployment. The public `install.ps1` bootstraps the server through WSL, not private desktop source. Preserve passwords, configuration and customer data. Preview snapshots are read-only reconciliation; on reload current runtime source is authoritative, cached source is fallback only after an explicit missing-session response.
...
The one-time isolation-copy script refuses existing targets and source units that are not confirmed loaded/inactive. Never remove newer workspace data to bypass that check. Rootless process cleanup happens after the attached client reads its exit status; do not reintroduce Podman's racing `--rm` option. The operator-only cPanel DNS hook reads separate root-owned `0600` configuration, modifies only allowlisted ACME TXT records and is not proof of live certificate issuance or renewal. See `docs/quality/2026-09-14-release-preparation.md`.

The combined beta is an opt-in GitHub test publication, not approval to migrate shared production. Consult `docs/releases/v4.1.0-beta.1.md` for its scope. Tag-based installers must repair the exact fetched commit, reject rewritten tags/divergence, and preserve secrets. Keep prereleases out of the stable/mandatory update channel and keep the native desktop release line independent.
