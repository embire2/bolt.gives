...
export class StreamRecoveryManager {
  ...
  private _deadlineHandle: NodeJS.Timeout | null = null;
  ...
  private _startDeadline() {
    const maxDuration = Number(this._options.maxDuration);

    if (!Number.isFinite(maxDuration) || maxDuration <= 0 || !this._isActive) {
      return;
    }

    this._deadlineHandle = setTimeout(() => {
      if (this._isActive) {
        this._deadlineHandle = null;
        logger.warn('Stream wall-clock deadline detected');
        this._handleTimeout();
      }
    }, maxDuration);
  }
  ...
  stop() {
    ...
    if (this._deadlineHandle) {
      clearTimeout(this._deadlineHandle);
      this._deadlineHandle = null;
    }
  }
  ...
  getStatus() {
    return {
      ...
      totalElapsed: this._startedAt === null ? 0 : Date.now() - this._startedAt,
    };
  }
}
