...
import { MAGNET_API_MODELS as FREE_HOSTED_MODELS } from '@bolt/agent/lib/modules/llm/magnet-api-provider-config';

const FREE_HOSTED_MODEL = 'gpt-5.6-sol';
const FREE_HOSTED_MODEL_LABEL = FREE_HOSTED_MODELS[0].label;
const FREE_HOSTED_MODEL_REASONING_EFFORT = 'medium';
...
describe('MagnetAPI transport compatibility', () => {
  afterEach(() => {
    vi.clearAllMocks();
    vi.unstubAllGlobals();
    vi.unstubAllEnvs();
  ...
  it('uses the dedicated BYOK MagnetAPI key for its Responses model', () => {
    ...
    const result = provider.getModelInstance({
      ...
      serverEnv: {
        MAGNETAPI_USER_API_KEY: 'magnet-test-key',
      } as unknown as Env,
    });
  ...
  it('routes ChatGPT-5.6 SOL through the Responses transport', () => {
    ...
    provider.getModelInstance({
      model: FREE_HOSTED_MODEL,
      serverEnv: { MAGNETAPI_USER_API_KEY: 'magnet-test-key' } as unknown as Env,
    });
  ...
  it('refuses to start when the dedicated server-side key is missing', () => {
    const provider = new MagnetApiProvider();
    vi.stubEnv('MAGNETAPI_USER_API_KEY', '');
  ...
  it('accepts a customer-supplied MagnetAPI key through apiKeys', () => {
    const provider = new MagnetApiProvider();
    const modelInstance = { id: 'free-model-instance' };
    responsesSpy.mockReturnValue(modelInstance);
  ...
  it('exposes the MagnetAPI BYOK fallback labels', () => {
    const provider = new MagnetApiProvider();
