...
import type { EditorDocument, ScrollPosition } from '@bolt/project/components/editor/codemirror/CodeMirrorEditor';
import type { ActionRunner } from '@bolt/project/lib/runtime/action-runner';
import type { ActionCallbackData, ArtifactCallbackData } from '@bolt/agent/lib/runtime/message-parser';
...
import type { ITerminal } from '@bolt/core/types/terminal';
import { EditorStore } from './editor';
import { FilesStore, type FileMap } from './files';
import { PreviewsStore } from './previews';
import type { PreviewInfo } from './previews';
import { TerminalStore } from './terminal';
import { extractRelativePath } from '@bolt/core/utils/diff';
import { description } from '@bolt/project/lib/persistence';
import { createSampler } from '@bolt/core/utils/sampler';
import type { ActionAlert, DeployAlert, SupabaseAlert } from '@bolt/core/types/actions';
...
import {
  ...
  isActionAutoAllowed,
  type AutonomyMode,
} from '@bolt/agent/lib/runtime/autonomy';
import { createResilientExecutionQueue } from '@bolt/core/lib/runtime/serial-execution-queue';
import { createScopedLogger } from '@bolt/core/utils/logger';
import { resolvePreferredArtifactFilePath } from '@bolt/core/lib/runtime/file-paths';
import {
...
export class WorkbenchStore {
  ...
  async addArtifact({ messageId, title, id, type }: ArtifactCallbackData) {
    ...
    const artifactPromise = (async () => {
      ...
      const actionRunnerModule = await import('@bolt/project/lib/runtime/action-runner');
  ...
  async downloadZip() {
    const { downloadWorkspaceZip } = await import('@bolt/project/lib/runtime/archive-export');
  ...
  async runTestAndSecurityScan() {
    ...
    try {
      ...
      const { runWorkspaceTestAndSecurityScan } = await import('@bolt/agent/lib/runtime/test-security-runner');
  ...
  async pushToRepository(
    ...
    try {
      ...
      const { pushWorkspaceToRepository } = await import('@bolt/project/lib/runtime/repository-publisher');
