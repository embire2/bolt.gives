...
function createDocConnection(port: number, roomName: string) {
  ...
  const webSocketClient = wsModule?.default ?? wsModule?.WebSocket ?? wsModule;
  ...
  const provider = new WebsocketProvider(`ws://127.0.0.1:${port}`, roomName, doc, {
    ...
    WebSocketPolyfill: webSocketClient as any,
  });
...
describe('collaboration-server', () => {
  ...
  it(
    'syncs edits across clients and restores persisted content after restart',
    async () => {
  ...
  it(
    'cleans up inactive documents after timeout when no clients remain',
    async () => {
      const port = await getFreePort();
      const persistDir = await mkdtemp(path.join(os.tmpdir(), 'bolt-collab-'));
      tempDirs.push(persistDir);
