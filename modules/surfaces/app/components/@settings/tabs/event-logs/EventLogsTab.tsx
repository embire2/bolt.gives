...
import { logStore, type LogEntry } from '@bolt/project/lib/stores/logs';
...
export function EventLogsTab() {
  ...
  const exportAsPDF = async () => {
    try {
      ...
      // Save the PDF
      doc.save(`bolt-gives-event-logs-${new Date().toISOString()}.pdf`);
  ...
  const exportAsText = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `bolt-gives-event-logs-${new Date().toISOString()}.txt`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
