import { isMac, isWindows, isLinux } from '@bolt/core/utils/os';
import { isMobile } from './mobile';
import { PROVIDER_LIST, DEFAULT_MODEL } from '@bolt/agent/utils/constants';
...
const getLogStore = () => {
  if (!logStore && typeof window !== 'undefined') {
    try {
      ...
      import('@bolt/project/lib/stores/logs')
        .then(({ logStore: store }) => {
          logStore = store;
        })
        .catch(() => {
  ...
  private async _getLogStoreLogs(): Promise<LogEntry[]> {
    try {
      ...
      if (!store) {
        ...
        try {
          const { logStore: storeModule } = await import('@bolt/project/lib/stores/logs');
