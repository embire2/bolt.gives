...
export function rewritePreviewAssetUrls(content, previewBasePath) {
  ...
  let rewritten = content
    ...
    .replace(
      /sourceMappingURL=\/(?!\/|runtime\/preview\/)(?=[@A-Za-z0-9_.-])/g,
      `sourceMappingURL=${previewBasePath}/`,
    );
  ...
  rewritten = rewritten
    .replace(/const serverHost = .*?;/, 'const serverHost = `${importMetaUrl.host}${proxyPathPrefix}`;')
    .replace(/const socketHost = .*?;/, 'const socketHost = `${importMetaUrl.host}${proxyPathPrefix}`;')
    .replace(/const directSocketHost = .*?;/, 'const directSocketHost = `${importMetaUrl.host}${proxyPathPrefix}`;')
...
export function extractPreviewPortFromOutput(text) {
  ...
  const matches = [
    ...normalizedText.matchAll(/https?:\/\/(?:127\.0\.0\.1|localhost|0\.0\.0\.0|\[::1\]):(\d+)(?:\/|\b)/gi),
  ];
