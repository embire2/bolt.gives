...
function assertRequiredEnv(env) {
  ...
  if (
    !String(env.BOLT_FREE_USAGE_QUOTA_SECRET || env.FREE_USAGE_QUOTA_SECRET || env.BOLT_HOSTED_FREE_RELAY_SECRET).trim()
  ) {
    missing.push('BOLT_FREE_USAGE_QUOTA_SECRET');
  }

  if (!String(env.BOLT_FREE_OPENROUTER_API_KEY || '').trim()) {
    missing.push('BOLT_FREE_OPENROUTER_API_KEY');
  }
...
export async function runCloudflareFreeProviderConfigCli() {
