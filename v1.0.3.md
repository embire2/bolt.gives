...
- [ ] Remove/disable heavy client-side work that causes slowdown on long projects.
- [ ] Move heavy tasks to server-side execution paths where possible:
...
- [ ] Keep UI responsive under long sessions by:
...
- [ ] Require Node runtime memory floor of at least 4096 MB for supported install paths.
- [ ] Ensure fresh installs and upgrades auto-apply memory defaults (no manual tuning required).
- [ ] Add install/upgrade preflight checks that verify `NODE_OPTIONS` includes `--max-old-space-size=4096` (or higher).
- [ ] Update installation docs and upgrade docs to reflect enforced baseline.

### P0.8 Git-Backed Auto-Update + One-Click Upgrade
- [ ] Add update monitor that checks remote Git version/tag on an interval and on app start.
- [ ] Display in-app update notice when a newer version is available.
- [ ] Provide one-click update flow that performs:
  - fetch latest
  - fast-forward/update to target version
  - dependency install
  - build/restart
- [ ] Include safe failure handling:
  - clear progress + logs
  - rollback/retry path if update fails
  - success confirmation with running version

### P0.9 Architect Self-Heal + Safety Guard
- [~] Ship Architect diagnosis + auto-heal scaffolding for known failure signatures.
- [ ] Expand Architect knowledgebase to cover top build/runtime/package-manager failures.
- [ ] Ensure Architect auto-heal is mode-aware and obeys safety/autonomy policy.
- [ ] Add structured recovery timeline events for Architect diagnosis, repair attempt, and outcome.

### P0.10 Cost Estimation Accuracy Across Providers
- [~] Normalize usage field mappings across provider responses.
- [ ] Ensure non-zero, accurate cost estimates in timeline/footer whenever usage exists.
- [ ] Add regression tests for usage normalization and cost calculation paths.
- [ ] Add E2E validation for strict + standard providers.

### P0.11 Provider/Model/API Key Session Persistence
- [~] Persist latest working API key/provider/model per instance.
- [ ] Auto-restore latest working selection on revisit.
- [ ] Keep provider history navigable so users can switch back to previously working providers.
- [ ] Add tests for persistence/restore and default-selection replacement rules.
...
- [ ] Performance soak test on long-run project flow (client responsiveness + memory growth budget).
- [ ] Install/upgrade integration test validating enforced Node memory floor.
- [ ] E2E update test: detect new version, show banner, apply one-click update, verify version bump.
- [ ] Unit/integration tests for Architect diagnosis routing and auto-heal gating by autonomy mode.
- [ ] E2E verification of persisted provider/model/key selection and non-zero cost estimation.
